            ...
            for(int output_depth=0;output_depth<output_depth_size;output_depth++)
            {
                ...
                for(int x=0;x<filter_dim1;x++)
                {
                    for(int y=0;y<filter_dim2;y++)
                    {
                        ...
                        {
                            filt_offset=compute_offset4D(x,y,z,output_depth,\
                                    filter_dim1,filter_dim2,filter_dim3,\
                                    filter_dim4);

                        img_offset=compute_offset3D(output_row+x,\
                                output_col+y,z,img_dim1,img_dim2,img_dim3);  
                        
                        op1=*((float *)(filter->data) + filt_offset);
                        op2=*((float *)(img->data) + img_offset);
                        sum=mac_op(sum,op1,op2);
                        }
                    }
                }
                //------------------------------Inner Loop Ends----------------
            layer_output_offset = compute_offset3D(output_row,output_col,output_depth,\
                    output_row_size,output_col_size,output_depth_size);
