...
void gate2(float x[], float h[], Param2_gate *weights, int activator, float gate_output[])
{
    ...
    float x_w[LAYER2_SIZE];    //x*w
    float h_w[LAYER2_SIZE];    //h*w
    float sum[LAYER2_SIZE];    //
    float pre_activation[LAYER2_SIZE];
    // Gate Computation
    // 1. M * x
    vmatmul(x, *(weights->w_x), HIDDEN_LAYER_SIZE, LAYER2_SIZE, x_w ) ;
    // 2. W * h
    vmatmul(h, *(weights->w_h), HIDDEN_LAYER_SIZE, LAYER2_SIZE, h_w );
    // 3. Wx + Wh
    vadd2(x_w,h_w, LAYER2_SIZE, sum);
    // 4. Wx + Wh + b
    vadd2(sum, *(weights->bias), LAYER2_SIZE, pre_activation);
    // 5. Activation
    if(activator)
        sigmoid(pre_activation, LAYER2_SIZE, gate_output);
    else
        tanhyp(pre_activation, LAYER2_SIZE, gate_output);    
}
