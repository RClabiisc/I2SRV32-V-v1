...
//int __attribute__((aligned(32))) __attribute__((section(".vmem_data")))  y_vec[VEC_SIZE];       // 1 x 32 Vector Result
...
{
    int a = 64;
    saxpy(y_vec, a, x_vec, VEC_SIZE);
    // Verify Results
    //int err_limit = 0x00028F5C;
    int err_limit = 10;
    err_count =0;   //Init the error counter for this layer to 0
    for(int i=0;i<VEC_SIZE;i++)                                     
        if( (( y_vec[i] - expected_output[i]) > err_limit) || ((y_vec[i] - expected_output[i]) < -err_limit))
           err_count++;                                              
    return 0;
}
